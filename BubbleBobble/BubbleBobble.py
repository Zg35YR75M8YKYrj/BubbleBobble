...
if sys.version_info < (3,5):
    print("This game requires a Python environment of 3.5 or above.")
    sys.exit

#Here we have a list comprehension to parse out the pgzero version...
pgzero_version = [int(s) if s.isnumeric() else s for s in pgzero.__version__.split('.')]

if pgzero_version < [1,2]:
    print("The pgzero version check failed, you need 1.2 or above, and you have {pgzero.__version__}")
    sys.exit
...
#Defining level data as a list data structure for now. Can probably automate generation later

#            1234567891111111111222222222       COLUMNS
#                     0123456789012345678  
LEVELS = [ ["XXXXX     XXXXXXXX     XXXXX",      #1
            "                            ",      #2
            "                            ",      #3
            "                            ",      #4
            "                            ",      #5
    ...
    def move(self, dx, dy, speed):
        ...
        for i in range(speed):
            ...
            if ((dy > 0 and new_y % GRID_BLOCK_SIZE == 0 or 
                 ...
                 dx < 0 and new_x % GRID_BLOCK_SIZE == GRID_BLOCK_SIZE -1 ) and
                block(new_x, new_y)):
                return True
...
CHAR_WIDTH = [27,26,25,26,25,25,26,25,12,26,26,25,33,25,26,25,27,26,26,25,26,26,38,25,25,25]
